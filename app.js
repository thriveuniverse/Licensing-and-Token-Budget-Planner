...
const UI = (() => {
        ...
        plans.forEach((plan, index) => {
            html += `
                <tr>
                    ...
                    <td><input type="number" step="0.0001" value="${plan.price_prompt_per_1k}" data-table="plans" data-index="${index}" data-key="price_prompt_per_1k"></td>
                    <td><input type="number" step="0.0001" value="${plan.price_completion_per_1k}" data-table="plans" data-index="${index}" data-key="price_completion_per_1k"></td>
                    <td><input type="number" step="1000" value="${plan.free_tier_tokens}" data-table="plans" data-index="${index}" data-key="free_tier_tokens"></td>
                    <td><input type="number" step="1" value="${plan.monthly_commit_credit}" data-table="plans" data-index="${index}" data-key="monthly_commit_credit"></td>
                    <td><input type="number" step="0.1" value="${plan.overage_multiplier}" data-table="plans" data-index="${index}" data-key="overage_multiplier"></td>
        ...
        envs.forEach((env, index) => {
            ...
            html += `
                <tr>
                    ...
                    <td>${selectHtml}</td>
                    <td><input type="number" value="${env.requests_per_day}" data-table="envs" data-index="${index}" data-key="requests_per_day"></td>
                    <td><input type="number" value="${env.avg_tokens_per_request}" data-table="envs" data-index="${index}" data-key="avg_tokens_per_request"></td>
                    <td><input type="number" value="${env.context_tokens}" data-table="envs" data-index="${index}" data-key="context_tokens"></td>
                    ...
                    <td><input type="number" value="${env.monthly_budget}" data-table="envs" data-index="${index}" data-key="monthly_budget"></td>
    ...
    function renderCharts(results) {
        ...
        // 1. Cost Chart
        let costHtml = '';
        const maxCost = Math.max(...perEnv.map(r => r.final_cost), 1); // Avoid div by zero
...
const App = (() => {
    ...
    function init() {
        ...
        // Listen for user inputs (delegated)
    ...
    /**
     * Handle all changes to <input> and <select> fields
     */
